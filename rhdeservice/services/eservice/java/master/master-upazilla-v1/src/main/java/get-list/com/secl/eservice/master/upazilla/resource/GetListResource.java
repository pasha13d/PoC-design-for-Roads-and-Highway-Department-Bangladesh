package com.secl.eservice.master.upazilla.resource;
...
import com.secl.eservice.master.upazilla.response.GetListResponseBody;
import com.secl.eservice.master.upazilla.service.GetListService;
...
@Slf4j
@RestController("masterUpazillaV1GetListResource")
@RequestMapping(Api.MASTER_UPAZILLA_V1_RESOURCE)
public class GetListResource {
	...
	@Autowired
	@Qualifier("masterUpazillaV1GetListService")
	private GetListService service;

	// @HystrixCommand(fallbackMethod = "fallbackGetAllOnPost")
	@PreAuthorize("securityHasPermission('"+Api.MASTER_UPAZILLA_V1_RESOURCE+ Api.MASTER_UPAZILLA_V1_GET_LIST_PATH+"')")
	@PostMapping(path = Api.MASTER_UPAZILLA_V1_GET_LIST_PATH, produces = { MediaType.APPLICATION_JSON_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE })
	public @ResponseBody GetListResponse onPost(@RequestBody GetListRequest request) throws AppException {
		String url = Api.MASTER_UPAZILLA_V1_RESOURCE.concat(Api.MASTER_UPAZILLA_V1_GET_LIST_PATH);
