...
public class StatusCheckQuery {
	
	@Synchronized
	public static ImmutablePair<String, Object[]> statusCheckSql(AuthUser user,StatusCheckRequest request){
	...
	@Synchronized
	public static ImmutablePair<String, Object[]> updateActiveInstance(AuthUser user,StatusCheckRequest request){
			List<Object> param = null;
			String sql = "";	
		if( request.getBody().getStatusCheck().getStep().equalsIgnoreCase("Next")) {
			param = Lists.newArrayList(request.getBody().getStatusCheck().getNextStep() , request.getBody().getStatusCheck().getOid());			
			...
			sql = " UPDATE " + Table.REQUISITION_INSTANCE + " set isActive = 'false', isDone = 'false' WHERE stepoid = '"+request.getBody().getStatusCheck().getFromstepOid() +"' and roadrequisitionoid = ? ";			
	...
	@Synchronized
	public static ImmutablePair<String, Object[]> updatedoneInstance(AuthUser user,StatusCheckRequest request){
			List<Object> param = null;
			String sql = "";	
		if( request.getBody().getStatusCheck().getStep().equalsIgnoreCase("Next")) {
			...
			param = Lists.newArrayList( request.getBody().getStatusCheck().getOid());
			sql = " UPDATE " + Table.REQUISITION_INSTANCE + " set isDone = 'false', isActive='true' WHERE stepoid = (select nextstep from "+Table.ROAD+" where oid ='"+request.getBody().getStatusCheck().getOid() +"') and roadrequisitionoid = ? ";			
	...
	@Synchronized
	public static ImmutablePair<String, Object[]> insertIntoStateSql(AuthUser user,StatusCheckRequest request, String oid){
