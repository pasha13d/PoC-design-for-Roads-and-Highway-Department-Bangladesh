...
import com.secl.eservice.authentication.registration.dao.GetListDao;
...
@Slf4j
@Service("authenticationRegistrationV1GetListService")
public class GetListService extends BaseService<GetListRequest, GetListResponse> {

	@Autowired
	@Qualifier("authenticationRegistrationV1GetListDao")
	...
	public GetListResponse perform(AuthUser user, String url, String version, GetListRequest request, GetListResponse response) throws AppException {
		try {
			List<GetList> registrationList = getListDao.getRegistrationAll();
			log.info("registrationList count - {}", registrationList.size());
			response.getBody().setData(registrationList);
		} catch (Exception e) {
