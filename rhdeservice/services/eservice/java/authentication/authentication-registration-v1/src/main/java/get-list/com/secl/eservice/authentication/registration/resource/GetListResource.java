...
import com.secl.eservice.authentication.registration.request.GetListRequest;
import com.secl.eservice.authentication.registration.response.GetListResponse;
import com.secl.eservice.authentication.registration.response.GetListResponseBody;
import com.secl.eservice.authentication.registration.service.GetListService;
...
@Slf4j
@RestController("authenticationRegistrationV1GetListResource")
...
public class GetListResource {
	...
	@Autowired
	@Qualifier("authenticationRegistrationV1GetListService")
	private GetListService service;

	// @HystrixCommand(fallbackMethod = "fallbackGetAllOnPost")
	@PreAuthorize("securityHasPermission('"+Api.AUTHENTICATION_REGISTRATION_V1_RESOURCE+ Api.AUTHENTICATION_REGISTRATION_V1_GET_LIST_PATH+"')")
	@PostMapping(path = Api.AUTHENTICATION_REGISTRATION_V1_GET_LIST_PATH, produces = { MediaType.APPLICATION_JSON_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE })
	public @ResponseBody GetListResponse onPost(@RequestBody GetListRequest request) throws AppException {
		String url = Api.AUTHENTICATION_REGISTRATION_V1_RESOURCE.concat(Api.AUTHENTICATION_REGISTRATION_V1_GET_LIST_PATH);
